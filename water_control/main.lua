-- main.lua
local srcPath = (debug.getinfo(1, "S").source or ""):gsub("^@", "")
local appDir = srcPath:match("^(.*)[/\\][^/\\]*$") or ""
if appDir ~= "" then
...
if type(cfgModule) ~= "table" then
    error("[致命] 配置模块加载失败：config.lua 可能为空或被截断，请重新运行安装器")
end
    ...
    while true do
        ...
        if businessTick then
            ...
            local ok, err = pcall(function()
                ...
                -- 数据聚合后自动存档（限频：默认 30 秒一次，避免频繁整表写盘拖慢主循环）
                if now - UI._lastPersistSave >= (CONFIG.PERSIST_SAVE_INTERVAL or 30) then
                    UI._lastPersistSave = now
                    pcall(persistence.save)
                end
        ...
        -- 无线广播开关：关闭时不发送任何广播（镜像端会停留在关闭前的数据）
        if net.isReady()
            and CONFIG.SYNC.BROADCAST_ENABLED ~= false
            and syncNow - UI._lastBroadcast >= CONFIG.SYNC.BROADCAST_INTERVAL then
